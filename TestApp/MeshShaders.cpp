...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PSTR cmdLine, int showCmd)
{
    ...
    window.CallDuringIdle([&](double elapsed_time_ms)
    {
        ...
        int draw_count = max_draw_count;//max(1u, min(uint32(1000*game_seconds), max_draw_count));
        *(commands["Draw"].repeats) = draw_count;
        ...
        for (int idx = 0; idx < draw_count; idx++)
        {
            ...
            wvp = XMMatrixMultiply(wvp, XMMatrixTranslation(cos(idx + game_seconds * 0.75) * 5, sin(idx + game_seconds * 0.6) * 5.5, -idx));
